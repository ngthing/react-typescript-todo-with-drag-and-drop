...
export const DragableTodoItem = (ps: TodoItemProps) => (
	<Draggable key={ps.id} draggableId={ps.id} index={ps.index}>
		{(provided) => (
			<ItemContainer
				key={ps.id}
				className={`${ps.isCompleted && "todo-is-completed"}`}
			...
			>
				...
				<ItemCheckbox
					className="checkbox"
					onClick={() => ps.toggleTodoCompleteAtIndex(ps.index)}
				>
					{ps.isCompleted && <span>&#x2714;</span>}
				</ItemCheckbox>
				<ItemTextContainer>
					<ItemText
						...
						wrap="off"
						value={ps.content}
						id={`todo-input-${ps.index}`}
						aria-label={`todo-input-${ps.index}`}
				...
				<Button
					...
					onClick={() => ps.handleDelete(ps.index)}
				>
					&#x2715;
				</Button>
			</ItemContainer>
